...
- **Status**: In Progress (2026-09-13). Delivered: `badge` / `tag` (keyboard close), `breadcrumb` (arrows + Enter navigation), `scroll_area` (arrows / PageUp / PageDown / Home / End scrolling), `splitter` (arrow resize, both directions), `window` (arrow move), `menu_bar` (Enter / ArrowDown opens the focused trigger, Tab walks the dropdown items, Enter activates, Escape closes), `table` (arrows move the highlighted row with auto-scroll, Enter activates). Remaining: `context_menu` (arrow navigation while open), `dock`, `rich_text`, `toast`.
- **Current State**:
...
| **a11y** | `FEAT-A11Y-01` | Keyboard Reachability for Container & Overlay Widgets | **P2** | In Progress (10 of 16 surfaces) |
