# 技术清单：缺陷 (bug) 与功能特性 (feat)

<p>
  <a href="ISSUE.md">English</a> · <a href="ISSUE_zh.md">简体中文</a>
</p>

本文档对代码库（`src/core`、`src/widgets`、`src/composite`、`src/draw`、`src/color`、`src/math` 及运行时演示页面）提供系统、客观的代码审查与缺陷清册。

全文分为四大主线：
- **第一部分：缺陷与技术债务 (`bug`)**：内存泄漏、状态错乱、未处理的输入通道、排版度量漂移及组件边缘情况遗漏。
- **第二部分：引擎能力与真实业务自举路线图 (`feat`)**：核心引擎扩展、排版增强，以及需用原生 MoonBit GUI 引擎彻底替换原生 HTML/DOM 的演示页面。
- **第三部分：结构解耦与架构模块化 (`arch`)**：子系统模块化、通用内存持久化、设计令牌分层、一等公民组件结构体、Painter 绘制抽象、物理包分层与演示舞台解耦。
- **第四部分：v0.5.1 之后全维度代码审查与审计清册 (`audit`)**：健壮性、可维护性、性能、技术文档、非代码业务自举、逻辑错误与 UX 交互影响。
...
优先级划分为：
- **P0**：内存泄漏、无界分配、状态损坏或算术崩溃。
- **P1**：核心子系统不完整、缺失标准输入通道或排版测量缺陷。
- **P2**：控件功能遗漏、可访问性障碍或边缘情况布局失效。
- **P3**：性能优化机会与渲染图元表达能力欠缺。
