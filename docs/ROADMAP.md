...
### 里程碑 3：视窗管理、高级容器与矩形裁剪栈（9月16日 – 9月18日）[已交付]
...
* **交付清单**：
  ...
  - [x] **全局顶层菜单栏** (`src/core/menu_bar.mbt`)：`menu_bar`, `menu`, `menu_item`, `menu_separator` 下拉弹出机制、桌面级 Hover-to-Switch 随动流转与前台图层遮挡拦截；
...
* **验收标准**：
  - [x] 多窗口层叠拖拽测试通过，下层视窗的控件不发生穿透点击，裁剪边界内外的指令被准确拦截；菜单栏下拉在无头测试与展厅中流转顺畅，外部点击与遮挡拦截完全生效。
