...
### 新增功能与优化
- **Steps 步骤条居中几何矢量对勾与叉号 (`src/widgets/steps.mbt`)**：
  - 将原本依赖字体字形的 `"✓"` 与 `"✕"` 字符替换为纯数学推导的几何线段图元 (`add_line`)，中心点与拐点基于圆心和半径严格对称计算。
  - 彻底杜绝跨平台字体基线偏移与高分屏缩放下的偏心缺陷，保证在任何分辨率与 DPI 下像素级居中。
...
- **官网展厅全景升级与代码工作区演进 (`examples/canvas/`)**：
  - 官网展厅全景接入 `VirtualList`（10,000 行列表视口切片）、`Spring Physics`（2D 阻尼弹簧小球回弹沙盒）与 `SvgExporter`（即时图元转标准 SVG）。
  - 全面升级 `code_workspace.html` 为纯粹全幅暗黑 IDE 风格，集成项目资源管理树、多标签实时代码编辑与交互式终端。
- **纯正引擎自举与界面纯净度规范 (`AGENTS.md`, `docs/DESIGN_SYSTEM*.md`)**：
  - 正式确立强制性规范：所有官方交互展示与工作台必须 100% 由纯 MoonBit 编写并通过 Canvas/Wasm 管线渲染，严禁 HTML/CSS DOM 仿真伪造，严禁无意义、脱节或色彩割裂的内嵌预览框。
- **自动化测试扩充**：
  - 新增 24 项白盒测试用例，覆盖物理模型、SVG 导出、气泡生命周期、虚拟列表切片、网格三角剖分以及 Steps 矢量几何对勾。
  - 全量自动化测试用例扩充至 **335 项，100% 验证通过**。
...
### 新增功能与优化
- **高性能流式表格与数据解析工具 (`src/widgets/table.mbt`)**：
  - 高性能流式表格数据结构：支持分隔符自动探测（逗号、制表符、分号）、带引号字段转义以及列数据类型自动推断（`INT`, `FLOAT`, `STR`）。
  - 零拷贝行间接索引表（`row_indices : Array[Int]`），实现 $O(1)$ 拷贝开销的多列就地快速排序与微秒级视口剔除虚拟滚动。
- **大规模数据基准测试套件**：
  - 新增 10,000 行与 50,000 行自生成基准测试用例，全流程验证数据加载、多列排序与全局过滤性能，100% 测试通过。
