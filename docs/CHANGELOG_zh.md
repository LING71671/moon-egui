...
- **WebGL 2.0 连续网格剖分与顶点缓冲管线 (`src/draw/mesh.mbt`)**：
  - 实现顶点数据结构 `Vertex`（2D 坐标 `x, y`、UV 坐标 `u, v`、归一化色彩通道 `r, g, b, a`）与网格连续缓冲 `Mesh`。
  - 核心几何图元三角网格化算法：`tessellate_rect`（直角与圆角扇形剖分）、`tessellate_circle`（圆弧三角扇）与 `tessellate_line`（沿法线方向挤出带状四边形）。
  - 合批生成器 `DrawList::to_mesh()` 与连续内存序列化函数 `Mesh::write_vertex_floats`，支持无拷贝直传 WebGL VBO 缓冲。
- **官网展厅新能力联动与轻量在线代码工作区原型 (`examples/canvas/`)**：
  - 官网展厅全景接入 `VirtualList`（10,000 行列表视口切片）、`Spring Physics`（2D 阻尼弹簧小球回弹沙盒）与 `SvgExporter`（即时图元转标准 SVG）。
  - 正式交付 `code_workspace.html` 轻量在线代码工作区原型，支持多文件资源管理树、多标签实时代码编辑、Canvas 2D 即时渲染视口、拖拽式弹性分栏器与交互式终端命令行。
- **自动化测试扩充**：
  - 新增 23 项白盒测试用例，覆盖物理模型、SVG 导出、气泡生命周期、虚拟列表切片与网格三角剖分。
  - 全量自动化测试用例扩充至 **334 项，100% 验证通过**。
...
### 新增功能与优化
- **MoonLens 独立原生桌面数据探索器（`app/moonlens`）**：
  - 基于 MoonBit 原生后端直接编译为 Win32 独立可执行程序（`moonlens.exe`），体积仅 286 KB，内存占用仅 ~2.6 MB。
  - 集成 Windows DWM 沉浸式暗黑标题栏，去除原生白色边框；实现任务栏与标题栏动态矢量图标；采用 Segoe UI 高分屏 ClearType 字体渲染。
  - 纯 GDI 双缓冲离屏绘制，稳定锁定 60 FPS 无闪烁刷新。
  - 实现惯性平滑阻尼滚动（Lerp 插值）与实时交互式搜索过滤输入栏（支持不区分大小写的高速子串匹配）。
  - 高性能流式数据解析引擎：支持分隔符自动探测（逗号、制表符、分号）、带引号字段转义以及列数据类型自动推断（`INT`, `FLOAT`, `STR`）。
  - 零拷贝行间接索引表（`row_indices : Array[Int]`），实现 $O(1)$ 拷贝开销的多列就地快速排序与微秒级视口剔除虚拟滚动。
  - 24 桶频率分布直方图计算器，支持分位数统计（`P25`、`P50` 中位数、`P75`、`P95`、总和）与悬停数据行数即时 Tooltip 查看。
- **大规模压力基准测试套件（`app/moonlens/csv_bench_wbtest.mbt`）**：
  - 新增 10,000 行与 50,000 行自生成基准测试用例，全流程验证数据加载、多列排序、全局过滤及分布直方图性能，100% 测试通过。
- **官方展示站点集成（`examples/canvas/index.html`）**：
  - 首页新增 MoonLens 原生桌面应用聚焦板块，提供技术规格、特性卡片、中英双语 i18n 支持与一键构建命令。

### 问题修复
- **快排最坏情况退化优化**：增加中位数基准元选择机制，防止预排序大数据集下递归退化为 $O(N^2)$。
- **零内存分配搜索匹配**：过滤匹配改用零分配字符扫描函数 `contains_ignore_case`，消除按键过滤时的瞬时垃圾回收开销。

## [0.3.1] - 2026-09-13

### 新增功能与优化
- **键盘可达性（`FEAT-A11Y-01`）**：菜单栏（触发器开合、下拉项遍历、Escape 关闭）、数据表格（方向键移动高亮行并自动滚入可视窗口、Enter 激活）、右键菜单（打开时方向键导航、跳过禁用项、Enter 展开子菜单）、DockArea（标签切换、分栏把手方向键调整）与独立 splitter、scroll_area、tag、breadcrumb、window 的键盘操作；新增 15 个多帧帧序列测试。
- **交互测试基建**：新增拖拽序列（press→drag→release）与受控组件回喂测试；`AGENTS.md` 固化「Widget Interaction Standard」规范（光标契约、命中测试、弹层前景作用域、全局缩放、键盘可达性、受控组件、多帧测试模式）。

### 问题修复
- 无（本版本为可访问性与测试覆盖补强）。

### 其他
- **README 重组**：主 `README.md` 改为英文（mooncakes.io 与 GitHub 默认展示），中文版迁移至 `README_zh.md`，双语切换链接保留。
- 测试总数 246 → 263（100% 通过）。
