...
(function () {
  ...
  function bindEvents() {
    ...
    function handleKeyDown(e) {
      ...
      // Intercept Command Palette (Ctrl+P / Cmd+P) and Save (Ctrl+S / Cmd+S)
      if ((e.ctrlKey || e.metaKey) && (e.key.toLowerCase() === 'p' || e.key.toLowerCase() === 'k' || e.key.toLowerCase() === 's')) {
        e.preventDefault();
      ...
      if (e.key.length > 1) {
        ...
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Tab', 'Enter', 'Backspace', 'Delete', 'Escape', 'Home', 'End', 'F5'].includes(e.key)) {
          e.preventDefault();
        }
    ...
    function handleKeyUp(e) {
      if (e.key.length > 1) {
        ...
        keysReleased.push(e.key.toLowerCase());
      }
    }
  ...
  function renderLoop() {
    if (window.moon_studio_step && ctx) {
      const curText = pendingText;
      const curPressed = keysPressed.slice();
      const curReleased = keysReleased.slice();
      const curFlags = modFlags;
      const curScroll = scrollDy;
      ...
      const dl = window.moon_studio_step(
        width,
        height,
        mouseX,
        mouseY,
      ...
      if (window.moon_studio_cursor) {
        const cur = window.moon_studio_cursor();
        if (cur && canvas.style.cursor !== cur) {
